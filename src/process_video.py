...
def process_video(video_path):
    ...
    if height >= 2160:
        ...
        resolucao_label = "1440p"
    elif height >= 1080:
        resolucao_label = "1080p"
    elif height >= 720:
        resolucao_label = "720p"
    elif height >= 480:
        resolucao_label = "480p"
    else:
        resolucao_label = f"{width}x{height}"
    ...
    for r in results:
        ...
        if r.boxes.id is not None:
            ...
            for cls, obj_id in zip(classes, ids):
                obj_id = int(obj_id)
    ...
    # O YOLO com stream=True pode não retornar os últimos frames do vídeo
    # por limitação interna do iterator, mesmo sem relação com detecções.
    # Esses frames são preenchidos com o último valor conhecido de ids_unicos
    # para manter a integridade do CSV acumulado.
